...
playlist_handling = TestList [
  ...
  runState (command "load f2 otherfile") emptyStore ~?= (True, Map.fromList [("f2", "otherfile")]),
  runState (loadf1 >> loadf2) emptyStore ~?= (True, Map.fromList [("f1", "soundfile"),("f2", "otherfile")])
  ]
  ]
  where
